...
public final class LazyIdList<T> extends AbstractList<T> implements Serializable {
	...
	public T get(final int index) {
		if (index < 0 || index >= lazyQueryView.size()) { throw new IndexOutOfBoundsException(); }
	...
	public int indexOf(final Object o) {
		...
		for (int i = 0; i < addedItems.size(); i++) {
			if (o.equals(addedItems.get(i).getItemProperty(idPropertyId).getValue())) { return i; }
		}
		// Check from mapping cache.
		if (idIndexMap.containsKey(o)) { return addedItems.size() + idIndexMap.get(o); }
		// Switching to brute forcing.
		for (int i = addedItems.size(); i < lazyQueryView.size(); i++) {
			if (o.equals(lazyQueryView.getItem(i).getItemProperty(idPropertyId).getValue())) { return i; }
		}
		// Not found.
		return -1;
	}
