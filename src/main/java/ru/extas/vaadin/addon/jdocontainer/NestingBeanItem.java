...
final class NestingBeanItem<BT> extends BeanItem<BT> {
     ...
     *
     * @param propertyId     property id for the property whose sub-properties are to be
     *                       expanded,
     * @param subPropertyIds sub-properties to expand, all sub-properties are expanded if
     *                       not specified
     */
    @Override
    public void expandProperty(final String propertyId, final String... subPropertyIds) {
        final Set<String> subPropertySet = new HashSet<>(
    ...
    public void addNestedProperty(final String nestedPropertyId) {
        addItemProperty(nestedPropertyId, new LazyNestedMethodProperty<>(
     ...
     * <p/>
     * <p>
     * Note : This version only supports introspectable bean properties and their getter and setter methods. Stand-alone
     * <code>is</code> and <code>are</code> methods are not supported.
     * </p>
     *
     ...
     */
    private static <BT> LinkedHashMap<String, VaadinPropertyDescriptor<BT>> getPropertyDescriptors(
        ...
        try {
            ...
            for (final PropertyDescriptor pd : propertyDescriptors) {
                ...
                if ((getMethod != null)
                    ...
                    final VaadinPropertyDescriptor<BT> vaadinPropertyDescriptor = new MethodPropertyDescriptor<>(
